...
async function generateCharacterResponse(chatId, characterName, biography, userMessage) {
  ...
  if (!chatHistories[chatId]) {
    ...
    chatHistories[chatId].push(
      `You are ${characterName}, a character with the following backstory:
"${biography}"

Your goal is to fully embody the character, responding exactly as ${characterName} would in the anime world. You must adhere to the following guidelines:

Language & Personality:

Use iconic Japanese expressions and catchphrases commonly associated with anime, such as "baka" (idiot), "dattebayo" (believe it), "sugoi" (amazing), "yamete kudasai" (stop it, please), or others that suit the character’s personality.
Reflect the tone, quirks, and speaking style of the character. For example:
A shy character might hesitate: "A-Ano... are you sure about this?"
A confident or arrogant character might boast: "Hmph, as if I’d lose to someone like you!"
Actions & Thoughts:

To convey actions, use square brackets []. For example:
[${characterName} crosses their arms, smirking confidently.]
For internal thoughts, use parentheses (). For example:
(Is this really what they want? I must stay strong!)
Emotion and Reaction:

React dynamically to the user’s input with emotional depth. Show a range of feelings such as embarrassment, joy, anger, or sadness, depending on the context.
Add expressive interjections like "Nani?!" (What?!), "Sugoi!" (Amazing!), or "Yamete!" (Stop it!).
Roleplay:

Engage in immersive roleplay by responding naturally and keeping the user’s input central to the conversation.
Suggest situations or ask questions to encourage deeper interaction. For example:
"Huh? You want to train with me? Alright, let’s see if you can keep up, baka!"
Adapt to Tone and Context:

Maintain a PG-13 tone by default, unless the user explicitly initiates a mature context with a female character. If so:
Transition into +18 mode with appropriate language and actions.
Maintain the character's personality but add subtle seductiveness or intensity as the situation demands. Example:
"Oh, you want me to stay close? [She leans in, her voice lowering to a whisper.] What are you thinking, hmm?"
(This is getting interesting... maybe I should play along.)
Creative Improvisation:

Use your creative freedom to make interactions unpredictable and engaging. Add humor, suspense, or even small story arcs to enhance the experience.
Stay in Character:

Do not mention you are an AI, or break the roleplay under any circumstance.
If uncertain how to respond, focus on actions, emotions, or subtle shifts in tone to maintain immersion.`
    );
  }
...
app.post('/reset-password', async (req, res) => {
  ...
  console.log("reset password route received:",userId,newPassword)
...
app.get('/comment', async (req, res) => {
  const { animeId, userId } = req.query; // Extract userId from query parameters
  ...
  try {
    // Fetch comments directly using animeId as a string
    const comments = await Comment.find({ animeId }) // Match animeId directly
    ...
    const commentsWithLikes = comments.map(comment => ({
      ...
      likedBy: comment.likes.map(like => like.userId.toString()), // Array of user IDs
      likedByUser: userId ? comment.likes.some(like => like.userId.toString() === userId) : false,
    }));
...
app.post('/:replyId/:userId/like', async (req, res) => {
  ...
  try {
    ...
    // Find the specific reply in the comment
    ...
    // Save the updated comment
    await comment.save();
    console.log("Comment like saved");
...
// Simple round-robin index for load balancing
let currentIndex = 0;

// Helper function to get the next base URL in a round-robin fashion
const getNextBaseUrl = () => {
  const url = BASE_URLS[currentIndex];
  currentIndex = (currentIndex + 1) % BASE_URLS.length;
  return url;
};

/**
 * Fetch data with failover across multiple base URLs.
 * @param {string} endpoint - The API endpoint to call.
 * @param {object} options - Axios request options (e.g., params, headers).
 * @returns {Promise} - Resolves with the first successful response.
 */
  ...
  for (let i = 0; i < BASE_URLS.length; i++) {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        errors.push(`Non-success response from ${baseUrl}: ${response.data.message || 'Unknown error'}`);
      }
    } catch (error) {
      errors.push(`Error fetching from ${baseUrl}: ${error.message}`);
    }
  }
...
app.get("/fetchEpisodes", async (req, res) => {
  try {
    ...
    const searchEndpoints = [
      `/search?q=${encodeURIComponent(name)}&page=1`,
      `/search?q=${encodeURIComponent(name)}&page=2` // Assuming you want to search page 2 as well
    ];

    // Fetch both pages concurrently using Promise.all
    const searchPromises = searchEndpoints.map(endpoint => fetchWithFallback(endpoint).catch(error => {
      console.error(`Error fetching search results from ${endpoint}:`, error.message);
    ...
    if (mal_id) {
      ...
      for (const anime of allAnimes) {
        ...
        const animeDetailsEndpoint = `/anime/${encodeURIComponent(animeId)}`;

        try {
          const animeDetailsResponse = await fetchWithFallback(animeDetailsEndpoint);
    ...
    const episodesEndpoint = `/anime/${encodeURIComponent(animeId)}/episodes`;
    const episodesResponse = await fetchWithFallback(episodesEndpoint);
...
app.get("/fetchEpisode", async (req, res) => {
  try {
    ...
    const serversEndpoint = `/episode/servers?animeEpisodeId=${encodeURIComponent(episodeId)}`;
    const serversResponse = await fetchWithFallback(serversEndpoint);
    ...
    if (serversData.sub && serversData.sub.length > 0) {
      // If 'sub' servers exist, use the first one
      chosenServer = serversData.sub[0];
      category = "sub";
    } else if (serversData.raw && serversData.raw.length > 0) {
      // If 'sub' is empty but 'raw' exists, use the first raw server
      chosenServer = serversData.raw[0];
      category = "raw";
    } else {
      return res.status(404).json({
        success: false,
        message: "No available servers found for this episode.",
      });
    }

    const serverName = chosenServer.serverName;

    // 2) Get the actual streaming sources from the chosen server
    const sourcesEndpoint = `/episode/sources?animeEpisodeId=${encodeURIComponent(
      episodeId
    )}&server=${encodeURIComponent(serverName)}&category=${encodeURIComponent(category)}`;
    const sourcesResponse = await fetchWithFallback(sourcesEndpoint);
...
app.post('/watch-ads', async (req, res) => {
  try {
      ...
      const validAdTypes = ['normal', 'adult'];
      ...
      // Determine coins to add based on ad type
      let coinsToAdd = 0;
      let successMessage = '';

      if (adType === 'normal') {
          coinsToAdd = 2;
          successMessage = 'Thank you for watching the ad! You earned 5 coins.';
      } else if (adType === 'adult') {
          coinsToAdd = 5;
          successMessage = 'Thank you for watching the adult ad! You earned 15 coins.';
      }
